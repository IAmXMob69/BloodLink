...
- The default SQLite file lives in `~/.local/share/bloodlink/` (or `server/data/` in a dev tree). Back it up; it is the whole server.

Known limits in 0.1:

- Voice is relayed by the host. There is no STUN/TURN.
...
- Sessions expire after `HEARTH_SESSION_DAYS` (default 7).
